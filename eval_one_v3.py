...
DEFAULT_CKPT = "outputs/checkpoints/jusdef_v2_full_hd512_s43.pt"
DEFAULT_TEST = "data/processed/graphs/test_graphs.pt"
DEFAULT_TAG = "jusdef_v2_full_hd512_s42"


def main():
    parser = argparse.ArgumentParser(
        description="Evaluate a single JusDef v2 checkpoint on the test set."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        ...
        help=f"Path to test graphs .pt file [default: {DEFAULT_TEST}]",
    )
    parser.add_argument(
        "--tag",
        type=str,
        default=DEFAULT_TAG,
        help=f"Tag used in the metrics filename [default: {DEFAULT_TAG}]",
    )
    ...
    test_graphs = torch.load(test_path, map_location="cpu")
    ...
    out = {
        ...
        "test_f1_seen": float(
            f1_score(
                targets[:, seen], preds[:, seen], average="macro", zero_division=0
            )
        ),
    ...
    out_path = f"outputs/logs/{tag}.json"
